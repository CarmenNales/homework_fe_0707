...
// Functie domeinnaam uit e-mailadres halen
function getEmailDomain(emailadress) {

    // Domeinnaam scheiden van het e-mailadres
    const domainName = emailadress.substring(emailadress.indexOf("@") + 1);
...
function typeOfEmail(emailadress) {
    ...
    // Onderscheid maken tussen studenten, medewerkers of externe e-mailadressen
...
// Functie voor het controleren of het e-mailadres valide is
function checkEmailValidity(emailadress) {

    // Variabelen met stellingen die gecheckt worden
    const checkAt = emailadress.includes("@");
    const checkComma = emailadress.includes(",");
    const checkDot = emailadress.lastIndexOf(".") === (emailadress.length - 1);

    // Controleren of de e-mailadressen aan de stellingen voldoen
    return !!(checkAt && !checkComma && !checkDot);
}
// Controleer de functie
